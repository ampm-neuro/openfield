...
%delete context label from the first and last 15 seconds of each iti
cut_time = 15; %seconds
for iti_ctxt = [10 11 12]
    
    %context index
    current_ctx = eptrials(:,6) == iti_ctxt;
    
    %find times
    start_iti = min(eptrials(current_ctx, 1));
    new_start_iti = start_iti + cut_time;
    end_iti = max(eptrials(current_ctx, 1));
    new_end_iti = end_iti - cut_time;
    
    %erase context ids outside bounds
    out_of_time_bounds = eptrials(:,1)<new_start_iti | eptrials(:,1)>new_end_iti;
    eptrials(current_ctx & out_of_time_bounds, 6) = nan;
end
