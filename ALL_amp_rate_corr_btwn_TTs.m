function [corr_vals, FRs_amps, all_cell_idx, all_cell_idx_bw, all_cell_idx_ia, tt_id, all_amplitudes] = ALL_amp_rate_corr_btwn_TTs(subjects, sessions)
...
path_origin = 'E:\Projects\InProgress\RSC-ContextCode\data\';
...
file_list_subjects = file_list_subjects(3:6);
...
for subject = 3:length_subjects %
    ...
    strcat(path_origin, num2str(file_names_subjects{:}(subject,1).name))
    ...
    for session = 1:length_sessions %sessions%
        ...
        if isequal(size(clusters), [2,1]) || isequal(size(clusters), [1,1])
            clusters = [];
            continue
        else
        ...
        if sum(cluster_idx) > 0
            ...
            tt_id = [tt_id; [repmat(all_sesh_num, size(clusters(:,1))) clusters(:,1)]]; 
    ...
    save(['corr_amp_btwn_tts_' rat '_' date])
