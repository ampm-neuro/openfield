function [rate_distributions, hd_counts] = hd_cell(varargin)
...
inst_hd = ceil(circ_smooth(eptrials(eptrials(:,4) == 1, 9), [0 360], angle_window));
...
for time_sample = 1:iteration_jump:length(mid_time_pts)
    ...
    if mean(eptrials(wndw_index,2))>ba(1) && mean(eptrials(wndw_index,2))<ba(2) && mean(eptrials(wndw_index,3))>ba(1) && mean(eptrials(wndw_index,3))<ba(2)
...
if isnan(sum(hd_counts))
    warning('missing hd values; data ommitted')
    rate_distributions = nan(size(clusters,1),360);
    info_content = nan(size(clusters(:,1)));
    return
end
