function [p_correct, assignments, classinput_ts_idx, correct_spaceerror_within, correct_spaceerror_between, chance_spaceerror] = ctxtspace_classify_crossbw_1(spk_cts, ctxt_idx, posx, posy, bins)
...
for ctxt = 1:2
    ...
    %spikes from correct-context-classified tws
    cspikes = classinput_s_cts(ccidx & caidx, :);
    ...
    [I_decode, J_decode] = ind2sub([bins, bins], cspikes);

    %calculate distance between decoded and actual at each time_window
    correct_spaceerror_within = [correct_spaceerror_within; diag(dist([I_decode,J_decode],[I_actual,J_actual]'))];
    
end



%find FR matrix for each context




%distances during incorrect classifications
correct_spaceerror_between = [];
for ctxt = 1:2
    ...
    %MD CLASS CROSSBW
    %
        %get FR matrix from opposite context
        o_ctxt = setdiff(ctxt, [1 2]);
        cspikes = classinput_s_cts(~ccidx & caidx, :);
    ...
    %calculate distance between decoded and actual at each time_window
    correct_spaceerror_between = [correct_spaceerror_between; diag(dist([I_decode,J_decode],[I_actual,J_actual]'))];
...
%plot
figure; hold on; 
histogram(correct_spaceerror_within.*(1/bins), (0:ceil(pdist([1 1; bins bins]))).*(1/bins), 'normalization', 'probability')
histogram(correct_spaceerror_between.*(1/bins), (0:ceil(pdist([1 1; bins bins]))).*(1/bins), 'normalization', 'probability')
%shuf range plot
