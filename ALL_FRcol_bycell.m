function [pvals, rvals, poly_vals] = ALL_FRcol_bycell
    ...
    for task = 2%:2
        ...
        for session = 1:length_sessions
            ...
            for c = 1:length(clusters)
                ...
                [r, p, poly_val] = fit_line(col_all, rates_all(:,c), dim);
                pvals = [pvals; p];
                rvals = [rvals; r];     
                poly_vals = [poly_vals; poly_val];
                
                axis([-1 1 min(rates_all(:,c))*.95 max(rates_all(:,c))*1.05])
                ...
                for ictxt = unique(ctxt_id)'
                   ...
                   %}
                   plot(col_all(ctxt_idx), rates_all(ctxt_idx,c), '.', 'markersize', 8)
